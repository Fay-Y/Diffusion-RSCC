...
class TransformerNetModel2(nn.Module):
    ...
    def __init__(
        ...
        self.num_heads = num_heads
        self.num_heads_upsample = num_heads_upsample
        print("logits_mode:",logits_mode)#logits_mode = 1
    ...
    def forward(self, x, timesteps, y=None, src_ids=None, src_mask=None):
        ...
        if self.num_classes is not None:
            assert y.shape == (x.shape[0],)
            emb = emb + self.label_emb(y) #类别标签的嵌入添加到时间步嵌入中
        ...
        emb_inputs = self.position_embeddings(position_ids) + emb_x + emb.unsqueeze(1).expand(-1, seq_length, -1)#计算输入的嵌入表示，包括位置编码、上采样投影和时间步嵌入。
