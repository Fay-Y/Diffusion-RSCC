...
from .transformerK import AutoConfig
...
class MiniClip(nn.Module):
    def __init__(self):
        super().__init__()
        Clip = CLIPModel.from_pretrained("/root/mj/DiffusionCC/remoteclip").cuda()
        self.vision_model = Clip.vision_model
    
    def forward(self, **x):
        vision_outputs = self.vision_model(**x)
        feature_map = vision_outputs[0]
        return feature_map
...
class image_clip(nn.Module):
    def __init__(self):
        super().__init__()
        self.image_embedding = MiniClip().cuda().train()
        self.linear = nn.Linear(50, 41, bias=True)
        
    def forward(self,x):

        x['pixel_values'] = x['pixel_values'].squeeze(1)
        x_feat = self.image_embedding(**x)
        x_feat = x_feat.permute(0,2,1)
        out = self.linear(x_feat)
...
class TransformerNetModel2(nn.Module):
    ...
    def forward(self, x, timesteps, cond1=None, cond2=None, y=None, src_ids=None, src_mask=None):
        """
        Apply the model to an input batch.
