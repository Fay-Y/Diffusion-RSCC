...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, cond1 = None,cond2 =None,clip_denoised=True, denoised_fn=None, model_kwargs=None
    ):
        """
        Apply the model to get p(x_{t-1} | x_t), as well as a prediction of
        the initial x, x_0.
        ...
        assert t.shape == (B,)
        # print(x.shape)
        model_output = model(x, self._scale_timesteps(t),cond1=cond1,cond2=cond2)
    ...
    def training_losses_e2e(self, model, w, t, cond1 = None, cond2 = None,  model_kwargs =None, noise=None):
        """
        Compute training losses for a single timestep.
        ...
        if self.loss_type == LossType.E2E_MSE or self.loss_type == LossType.E2E_RESCALED_MSE:
            model_output = model(x_t, self._scale_timesteps(t), cond1=cond1, cond2=cond2)
            target = {
                ModelMeanType.PREVIOUS_X: self.q_posterior_mean_variance(
                    x_start=x_start, x_t=x_t, t=t
                )[0],
