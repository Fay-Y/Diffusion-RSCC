...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        self.model_params = list(self.model.parameters()) #delete layer without gradd
    ...
    def run_loop(self):
        ...
        while True:
            epoch = epoch+1
            for cond1,cond2,batch,_ in self.data:
                cond1,cond2 = self.model.img_process(cond1.cuda(),cond2.cuda())
                self.run_step(batch, cond1, cond2)
                        
                if self.step % self.log_interval == 0:
                    print("running epoch:",epoch,"run steps:",self.step)
    ...
    def forward_only(self, batch, cond1, cond2):
        with th.no_grad():
