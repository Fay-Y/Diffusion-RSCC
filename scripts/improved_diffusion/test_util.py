...
def compute_logp(args, model, x, input_ids):
    #根据模型的词嵌入和输入数据，计算文本数据的对数概率密度函数，并返回相应的损失。
    ...
    logp_expanded = logp_expanded.permute((1, 0)) #logp_expanded是一个二维张量，每一行代表一个时间步的概率密度函数对数。
    # print(th.topk(logp_expanded.view(bsz, seqlen, -1), k=5, dim=-1)[0])
    # print(input_ids[0])
    ce = th.nn.CrossEntropyLoss(reduction='none')
    loss = ce(logp_expanded, input_ids.view(-1)).view(bsz, seqlen) #loss一个二维张量，每一行代表一个时间步的交叉熵损失。
    # print(loss[0])
...
def get_weights(model, args):
    if hasattr(model, 'transformer'):
        ...
        print("down_proj_emb.shape:",down_proj_emb.shape)
        ...
        print("args.emb_scale_factor:",args.emb_scale_factor)
