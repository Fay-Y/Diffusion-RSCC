...
from dataloader import CCDataset#,load_data_CC
import time
def main():
    ...
    if args.modality == 'image':
        ...
        data_folder = "/root/yxf/RSCC-Diffusion/datasets"

        dataset = CCDataset(data_folder, 'train')
        data =  DataLoader(dataset, batch_size=args.batch_size, shuffle=True, num_workers=0, drop_last=False, pin_memory=False)
        dataset_valid = CCDataset(data_folder, 'val')
        data_valid = DataLoader(dataset_valid, batch_size=args.batch_size, shuffle=True, num_workers=0, drop_last=False, pin_memory=False)
